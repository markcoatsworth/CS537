...
void* Mem_Alloc(int size)
{
	...
	int AllocSize = size;
	int SpaceAvailable = Mem_Available();
	...
	// Make sure enough space is available. If not, exit.
	//printf("[Mem_Alloc] SpaceAvailable=%d, AllocSize=%d\n", SpaceAvailable, AllocSize);
	if(SpaceAvailable < AllocSize)
	{
		return NULL;
	}
	...
	if((TargetNode->Size != AllocSize) && TargetNode->Size < (AllocSize + sizeof(AllocNode)))
	{
		return NULL;
	}
...
int Mem_Available()
{
	...
	// Iterate through the list, counting all free space
	AllocNode* ThisNode = AllocListHead;
	while(ThisNode != NULL)
	{
		if(ThisNode->IsAllocated == 0)
		{
			AvailableSpace += ThisNode->Size;
		}
		ThisNode = ThisNode->Next;
	}
